...
| Variable de entorno | Obligatoria | Qué hace |
|---|---|---|
| `NEVENT_JWT_TOKEN` | Sí | Autentica el servidor MCP con tu cuenta de Nevent. |
| `NEVENT_OPERATION_MODE` | No | Controla qué puede hacer el asistente: `READ_ONLY` (por defecto, solo consulta), `STANDARD` o `FULL`. Ver [permisos y seguridad](/nevent-ai/permisos-y-seguridad). |
